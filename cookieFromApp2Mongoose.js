...
//쿠키를 Set 해보기위한 라우트       //.post(upload.array('photo', 1)) 파일을 업로드 하기위한 기본설정
router.route("/process/setCookie").post(upload.array('photo', 1),function(req, res){
    ...
    var name = req.body.name || req.query.name;
    var id = req.body.id || req.query.id;
    var nick = req.body.nick || req.query.nick;
    // 쿠키를 Set 하기 위한 설정 아래 'user'는 쿠키의 대표이름 이다. req.cookies.user(앞에 설정한 값).원하는 쿠키값.
    res.cookie('user', {
      cookieName:name,
      cookieId:id,
      cookieNick:nick,
...
router.route("/process/showCookie").get(function(req, res){
  ...
  //var userName = 'user'+cnt;
  res.writeHead(200, { "Content-Type":"text/html;charset=utf8"});
  res.write('<div><img src="/upload/'+req.cookies.user.filename+'" width="300" height="300"><div>');
  res.write(req.cookies.user.cookieName+", <br>"+req.cookies.user.cookieId+", <br>"+req.cookies.user.cookieNick+"<br><a href='/public/receiveCookie.html'>receiveCookie</a>");
  res.end();
})
